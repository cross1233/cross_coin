...
exports.APTOS_DOMAIN_ID = exports.BASE_SEPOLIA_CONFIG = exports.BaseCCTPSender = void 0;
exports.evmToAptosAddress = evmToAptosAddress;
exports.aptosAddressToBytes32 = aptosAddressToBytes32;
...
exports.BASE_SEPOLIA_CONFIG = BASE_SEPOLIA_CONFIG;
// Aptos测试网域ID
const APTOS_DOMAIN_ID = 9;
exports.APTOS_DOMAIN_ID = APTOS_DOMAIN_ID;
 ...
 */
function aptosAddressToBytes32(aptosAddress) {
    // 确保是64个字符（32字节）
 ...
 * Base链CCTP跨链发送器类
 */
class BaseCCTPSender {
    constructor() {
        // 初始化Base Sepolia提供者
        this.provider = new ethers_1.ethers.JsonRpcProvider(BASE_SEPOLIA_CONFIG.rpcUrl);
        // 初始化合约实例（只读模式）
        this.tokenMessengerContract = new ethers_1.ethers.Contract(BASE_SEPOLIA_CONFIG.contracts.tokenMessengerV2, TOKEN_MESSENGER_ABI, this.provider);
        this.usdcContract = new ethers_1.ethers.Contract(BASE_SEPOLIA_CONFIG.contracts.usdc, USDC_ABI, this.provider);
    }
    /**
     * 检查用户USDC余额
     */
    async checkUSDCBalance(userAddress) {
        try {
    ...
    async approveUSDC(params) {
        try {
            ...
            // 获取USDC精度
            const decimals = await this.usdcContract.decimals();
            const amountWei = ethers_1.ethers.parseUnits(params.amount, decimals);
            // 批准TokenMessenger合约使用用户的USDC
            // 为了避免授权不足，我们授权一个更大的金额
            const maxUint256 = ethers_1.ethers.MaxUint256;
            const approveTx = await usdcWithSigner.approve(BASE_SEPOLIA_CONFIG.contracts.tokenMessengerV2, maxUint256 // 授权最大金额，避免重复授权
            // 让ethers自动处理nonce
            );
    ...
    async depositForBurn(params) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                // 调用depositForBurn函数
                const tx = await contractWithSigner.depositForBurn(amountWei, // 要烧毁的USDC数量
     ...
     * 一键完成：检查余额 -> 批准 -> 跨链转账
     */
    async executeFullCrossChain(params) {
        try {
...
exports.BaseCCTPSender = BaseCCTPSender;
//# sourceMappingURL=base-sender.js.map