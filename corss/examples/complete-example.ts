...
import { crossChainOrchestrator, FullCrossChainParams } from '../src/cross-chain-orchestrator';
...
const CONFIG = {
  ...
  APTOS_RECIPIENT: '0xba916e9cbf294e552e4281dfc227fadbd3413d81c2fc233816f85dd89d53f54c',            // Aptos接收地址（修正为私钥对应的地址）
...
async function runCrossChainExample() {
  ...
  try {
    ...
    const crossChainParams: FullCrossChainParams = {
    ...
    const prerequisites = await crossChainOrchestrator.checkPrerequisites(crossChainParams);
    ...
    // ========== 3. 费用估算 ==========
    console.log('💰 估算跨链费用...');
    const costEstimate = await crossChainOrchestrator.estimateCrossChainCost();
    ...
    const result = await crossChainOrchestrator.executeCrossChain(crossChainParams);
...
async function checkBalancesExample() {
  ...
  try {
    ...
    const aptosBalance = await aptosCCTPReceiver.checkUSDCBalance(CONFIG.APTOS_RECIPIENT);
...
function createAptosAccountExample() {
  ...
  console.log(`私钥: ${(account as any).privateKey.toString()}`);
